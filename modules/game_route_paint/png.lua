-- Minimal PNG writer for the paint overlay: 8-bit RGBA, zlib "stored" blocks (no compression).
...
-- pixelAt(x, y) -> r, g, b, a (0..255). a == 0 is fully transparent. Kept for small images.
function RoutePNG.encode(w, h, pixelAt)
  local rows = {}
  for y = 0, h - 1 do
    local row = { "\0" }
    for x = 0, w - 1 do
      local r, g, b, a = pixelAt(x, y)
      row[#row + 1] = schar(r - r % 1, g - g % 1, b - b % 1, a - a % 1)
    end
...
-- rowAt(y) -> the row's pixels as one string of exactly w*4 bytes. Building a row out of a handful of
-- string.rep runs is several times cheaper than calling back per pixel, which is what the paint layer does.
function RoutePNG.encodeRows(w, h, rowAt)
  local rows = {}
  for y = 0, h - 1 do
    rows[#rows + 1] = "\0" .. rowAt(y)
  end
  local ihdr = u32(w) .. u32(h) .. schar(8, 6, 0, 0, 0)
  return "\137PNG\r\n\26\n" .. chunk("IHDR", ihdr) .. chunk("IDAT", zlibStored(concat(rows))) .. chunk("IEND", "")
end

function RoutePNG.pixel(r, g, b, a) return schar(r, g, b, a) end
