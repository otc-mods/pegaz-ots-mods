...
targetbotMacro = macro(100, function()
  ...
  -- One flood-fill from the player per tick; each creature's path length is then a lookup.
  -- (Stock code ran a full flood per creature, players and NPCs included: 10+ floods per 100 ms.)
  local paths = findAllPaths(pos, 7, {ignoreNonPathable=true, ignoreCost=true})
  local function stepsTo(key)
    local node = paths[key]
    if not node then return nil end
    local steps = 0
    while node and node[3] >= 0 do
      steps = steps + 1
      node = paths[node[4]]
    end
    return steps
  end
  -- steps to the creature's tile; if the creature itself blocks it, shortest neighbour + 1 (same as ignoreLastCreature)
  local function pathLength(cpos)
    local direct = stepsTo(cpos.x .. "," .. cpos.y .. "," .. cpos.z)
    if direct then return direct end
    local best
    for dx = -1, 1 do
      for dy = -1, 1 do
        if dx ~= 0 or dy ~= 0 then
          local s = stepsTo((cpos.x + dx) .. "," .. (cpos.y + dy) .. "," .. cpos.z)
  ...
  for i, creature in ipairs(creatures) do
    local path = creature:isMonster() and pathLength(creature:getPosition()) or nil
    if path then
