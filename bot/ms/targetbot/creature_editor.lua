TargetBot.Creature.edit = function(config, callback) -- callback = function(newConfig)
  ...
  local addScrollBar = function(id, title, min, max, defaultValue)
    ...
    return widget                       -- callers may want to add a tooltip
  end
  ...
  addScrollBar("lureFollowDist", "Lure: max gap", 1, 6, 3)
  local sortBar = addScrollBar("targetSort", "Order (0def 1lo 2hi 3nr 4far)", 0, 4, 0)
  if sortBar then
    sortBar:setTooltip("0 = stock priority, 1 = lowest HP, 2 = highest HP, 3 = nearest, 4 = farthest")
  end
  ...
  -- "Use ..." flags apply in attack mode Rule; AoE / Single modes use the spell texts directly
