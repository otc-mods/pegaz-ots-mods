-- Master sorcerer defaults. This is the only file that differs between the shipped presets: it seeds spell words,
...
seed("healing1", { on = false, title = "HP%", text = "exura vita", min = 51, max = 90 })
seed("healing2", { on = false, title = "HP%", text = "exura gran", min = 0, max = 50 })
...
seed("manaitem2", { on = false, title = "MP%", item = 7590, min = 0, max = 50 })
...
if type(storage.buffs) ~= "table" then
  storage.buffs = { { text = "", interval = 10 }, { text = "", interval = 10 } }
end
...
VOCATION_ICONS = {
  { item = 238, lmb = "useSelf", note = "great mana potion" },
  { item = 3155, lmb = "crosshair", note = "sudden death rune" },
