-- Royal paladin defaults. This is the only file that differs between the shipped presets: it seeds spell words,
...
seed("healing1", { on = false, title = "HP%", text = "exura san", min = 51, max = 90 })
seed("healing2", { on = false, title = "HP%", text = "exura gran san", min = 0, max = 50 })
...
if type(storage.buffs) ~= "table" then
  storage.buffs = { { text = "utito tempo san", interval = 10 }, { text = "", interval = 10 } }
end
...
VOCATION_ICONS = {
  { item = 7642, lmb = "useSelf", note = "great spirit potion" },
